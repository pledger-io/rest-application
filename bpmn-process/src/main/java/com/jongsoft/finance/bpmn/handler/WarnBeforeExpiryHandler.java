...
class WarnBeforeExpiryHandler implements CommandHandler<WarnBeforeExpiryCommand> {
    ...
    WarnBeforeExpiryHandler(
            ProcessEngine processEngine, AuthenticationFacade authenticationFacade) {
    ...
    public void handle(WarnBeforeExpiryCommand command) {
        log.trace(
                "[{}] - Starting the BPMN process to warn before contract expires.", command.id());
