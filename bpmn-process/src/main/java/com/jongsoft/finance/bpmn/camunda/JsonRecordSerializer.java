...
public class JsonRecordSerializer<T> extends AbstractTypedValueSerializer<TypedValue> {
  ...
  public TypedValue convertToTypedValue(UntypedValueImpl untypedValue) {
    logger.trace(
        "Converting untyped value to typed value: {}",
        untypedValue.getValue().getClass().getSimpleName());
  ...
  public TypedValue readValue(ValueFields valueFields, boolean b, boolean b1) {
    ...
    try {
      return Variables.objectValue(
              objectMapper.readValue(new String(valueFields.getByteArrayValue()), supportedClass))
          .serializationDataFormat("application/json")
          .create();
    } catch (IOException e) {
  ...
  protected boolean canWriteValue(TypedValue typedValue) {
    if (typedValue instanceof UntypedValueImpl) {
      logger.trace(
          "Checking if un-typed value can be written: {}",
