...
/**
 * This delegate allows for locating a specific budget in the system using one of the following
 * methods:
 *
 * <ol>
 *   <li><strong>name</strong>, the unique name of the budget
 * </ol>
 *
 * The output of this delegate will be:
 *
 * <ul>
 *   <li>{@code budget}, the {@link Budget} found
 * </ul>
 */
@Slf4j
@Singleton
public class ProcessBudgetLookupDelegate implements JavaDelegate, JavaBean {
  ...
  public void execute(DelegateExecution execution) {
    log.debug(
        "{}: Processing budget lookup '{}'",
        execution.getCurrentActivityName(),
        execution.getVariableLocal("name"));

    var filter = filterFactory.expense().name((String) execution.getVariableLocal("name"), true);
    var expense = expenseProvider.lookup(filter);
    if (expense.total() == 0) {
      throw new IllegalStateException(
          "Budget cannot be found for name " + execution.getVariableLocal("name"));
    }
