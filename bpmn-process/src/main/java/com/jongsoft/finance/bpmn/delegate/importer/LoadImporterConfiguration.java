...
public class LoadImporterConfiguration implements JavaDelegate, JavaBean {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        importerProvider.stream()
                .filter(importer -> importer.getImporterType()
                        .equalsIgnoreCase(importJob.getConfig().getType()))
                .findFirst()
                .ifPresentOrElse(
                        importer -> delegateExecution.setVariableLocal(
                                ...
                                new ImportJobSettings(
                                        ...
                                        null)),
                        () -> {
                            throw new IllegalStateException("Cannot find importer for type "
                                    + importJob.getConfig().getType());
                        });
    }
}
