...
public class ParseUserConfigurationDelegate implements JavaDelegate, JavaBean {
    ...
    public void execute(DelegateExecution execution) throws Exception {
        ...
        if (profileJson.getRules() != null && !profileJson.getRules().isEmpty()) {
            String ruleStorageToken = storageService.store(mapper.writeSafe(RuleConfigJson.builder()
                            .slug("profile-import")
        ...
        execution.setVariableLocal(
                "transactions",
                Control.Option(profileJson.getTransactions()).getOrSupply(List::of));
        ...
        execution.setVariableLocal(
                "tags", Control.Option(profileJson.getTags()).getOrSupply(List::of));
    }
