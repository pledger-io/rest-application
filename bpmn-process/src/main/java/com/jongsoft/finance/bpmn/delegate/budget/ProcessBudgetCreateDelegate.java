...
/**
 * This delegate will create process a serialize {@link BudgetJson} into a new budget period in the
 * system. It will either create the first budget or close the existing and create a new one.
 *
 * <p>This delegate expects the following variables to be present:
 *
 * <ul>
 *   <li>budget, a JSON serialized {@link BudgetJson} to be created in the system
 * </ul>
 */
@Slf4j
@Singleton
public class ProcessBudgetCreateDelegate implements JavaDelegate, JavaBean {
    ...
    ProcessBudgetCreateDelegate(BudgetProvider budgetProvider, ProcessMapper mapper) {
    ...
    public void execute(DelegateExecution execution) {
        ...
        if (oldBudget.isPresent()) {
            log.debug(
                    "{}: Budget period already exists for period '{}' with start '{}'",
                    execution.getCurrentActivityName(),
                    start,
        ...
        budgetJson
                ...
                .forEach(e -> Control.Option(budget.determineExpense(e.getName()))
                        .ifPresent(
                                currentExpense -> currentExpense.updateExpense(e.getUpperBound()))
                        .elseRun(() -> budget.createExpense(
