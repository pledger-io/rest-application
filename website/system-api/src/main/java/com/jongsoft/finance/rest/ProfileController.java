...
public class ProfileController implements ProfileApi {
  ...
  public ProfileController(
      UserProvider userProvider, AuthenticationFacade authenticationFacade, FinTrack application) {
  ...
  public byte @Nullable(inherited = true) [] generateQrCode(String user) {
    ...
    return userProvider
        ...
        .getOrThrow(() -> StatusException.internalError("Cannot correctly determine user."));
  }
  ...
  public HttpResponse<Void> patch2Factor(
    ...
    switch (patchMultiFactorRequest) {
      case EnableMfaRequest e -> {
        if (!TwoFactorHelper.verifySecurityCode(userAccount.getSecret(), e.getVerificationCode())) {
