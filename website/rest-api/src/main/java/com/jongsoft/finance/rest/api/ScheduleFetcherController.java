...
class ScheduleFetcherController implements ScheduleFetcherApi {
    ...
    public ScheduleFetcherController(TransactionScheduleProvider scheduleProvider, FilterFactory filterFactory) {
    ...
    @Override
    public List<@Valid TransactionScheduleResponse> findScheduleByFilter(List<@NotNull Integer> account, List<@NotNull Integer> contract) {
        ...
        if (contract != null && !contract.isEmpty()) {
            filter.contract(Collections.List(contract.stream()
                  .map(id -> new EntityRef((long)id))
                  .toList()));
        }
