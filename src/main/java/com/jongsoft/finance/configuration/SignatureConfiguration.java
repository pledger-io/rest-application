...
@Factory
class SignatureConfiguration implements RSASignatureGeneratorConfiguration {
    ...
    public SignatureConfiguration(
            @Value("${micronaut.application.storage.location}/rsa-2048bit-key-pair.pem")
                    String pemPath) {
    ...
    static Optional<KeyPair> keyPair(String pemPath) {
        ...
        try (var pemParser =
                new PEMParser(new InputStreamReader(Files.newInputStream(Paths.get(pemPath))))) {
