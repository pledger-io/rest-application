...
public class Encryption {
  ...
  public synchronized byte[] decrypt(byte[] base64CiphertextAndNonceAndSalt, String password) {
    try {
      ...
      System.arraycopy(
          cipherTextAndNonceAndSalt, salt.length, cipherTextAndNonce, 0, cipherTextAndNonce.length);

      // Create an instance of PBKDF2 and derive the key.
      var pwSpec =
          new PBEKeySpec(password.toCharArray(), salt, PBKDF2_ITERATIONS, ALGORITHM_KEY_SIZE);
      var keyFactory = SecretKeyFactory.getInstance(PBKDF2_NAME);
