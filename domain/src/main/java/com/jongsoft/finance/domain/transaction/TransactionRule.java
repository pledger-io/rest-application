...
public class TransactionRule implements AggregateBase {
    ...
    public class Condition implements Serializable, Removable {
        ...
        public void update(RuleColumn field, RuleOperation operation, String condition) {
            ...
            if (hasChanged) {
                ...
                EventBus.getBus().send(new RuleConditionUpdatedEvent(this, id, field, operation, condition));
            }
        }
    ...
    public class Change implements Serializable, Removable {
        ...
        public void update(RuleColumn ruleColumn, String change) {
            ...
            if (hasChanged) {
                ...
                EventBus.getBus().send(new RuleChangeUpdatedEvent(this, id, ruleColumn, change));
            }
        }
    ...
    @Generated
    public TransactionRule(Long id, String name, String description, boolean restrictive, boolean active,
                           boolean deleted, String group, int sort, List<Condition> conditions,
                           List<Change> changes, UserAccount user) {
        this.id = id;
        this.name = name;
    ...
    public void changeOrder(int sort) {
        if (this.sort != sort) {
            this.sort = sort;
            if (id != null) {
                EventBus.getBus().send(new TransactionRuleSortedEvent(this, id, sort));
            }
        }
    }
    ...
    public Change findChange(long changeId) {
        return this.changes
                .first(c -> Objects.equals(c.getId(), changeId))
                .get();
    }
}
