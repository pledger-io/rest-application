...
public class Budget implements AggregateBase {
  ...
  public class Expense implements AggregateBase {
    ...
    /**
     * Create an expense and bind it to its parent budget. This will not register the expense in the
     * system yet.
     */
  ...
  void activate() {
    if (id == null && !active) {
      active = true;
      CreateBudgetCommand.budgetCreated(this);
    }
  }
  ...
  void close(LocalDate endDate) {
    if (this.end != null) {
      throw StatusException.badRequest(
          "Already closed budget cannot be closed again.", "validation.budget.already.closed");
    }
