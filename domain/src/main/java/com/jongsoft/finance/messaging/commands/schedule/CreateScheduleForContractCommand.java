...
public record CreateScheduleForContractCommand(
        String name, Schedule schedule, Contract contract, Account source, double amount)
    ...
    public static void scheduleCreated(
            String name, Schedule schedule, Contract contract, Account source, double amount) {
        new CreateScheduleForContractCommand(name, schedule, contract, source, amount).publish();
    }
}
