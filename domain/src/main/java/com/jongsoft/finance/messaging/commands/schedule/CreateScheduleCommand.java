...
public record CreateScheduleCommand(String name, Schedule schedule, Account from, Account destination, double amount)
    ...
    public static void scheduleCreated(String name, Schedule schedule, Account from, Account destination, double amount) {
        new CreateScheduleCommand(name, schedule, from, destination, amount)
                .publish();
    }
