...
/**
 * Represents a command to change contract information.
 * This record includes the contract's ID, name, description, start date, and end date.
 * Implements the ApplicationEvent interface to allow publishing events.
 * Provides a static method contractChanged to create an instance of ChangeContractCommand and publish it.
 */
public record ChangeContractCommand(long id, String name, String description, LocalDate start, LocalDate end)
     ...
     */
    public static void contractChanged(long id, String name, String description, LocalDate start, LocalDate end) {
        new ChangeContractCommand(id, name, description, start, end)
                .publish();
    }
}
