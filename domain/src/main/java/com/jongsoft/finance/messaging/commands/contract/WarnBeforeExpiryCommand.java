...
public record WarnBeforeExpiryCommand(long id, LocalDate endDate) implements ApplicationEvent {
    ...
    /**
     * Publishes a warning before the expiry date of a certain item identified by the given ID. The warning will be sent
     * as an event through the application's event bus.
     *
    ...
    public static void warnBeforeExpiry(long id, LocalDate endDate) {
        new WarnBeforeExpiryCommand(id, endDate)
                .publish();
    }
}
