...
public class BalanceResource {
    ...
    @Operation(
            ...
            description =
                    "This operation will calculate the balance for the current user based upon the"
                            + " given filters",
    ...
    @Operation(
            ...
            description =
                    "Partition all transaction matching the balance request using the partitionKey"
                            + " provided.",
            ...
            parameters = {
                @Parameter(
                        ...
                        description =
                                "The partition key can be one of: account, budget or category")
            })
    ...
    private TransactionProvider.FilterCommand buildFilterCommand(BalanceRequest request) {
        ...
        if (!request.getAccounts().isEmpty()) {
            filter.accounts(
                    Collections.List(request.getAccounts()).map(a -> new EntityRef(a.getId())));
        } else {
            filter.ownAccounts();
        }
        ...
        if (!request.getExpenses().isEmpty()) {
            filter.expenses(
                    Collections.List(request.getExpenses()).map(a -> new EntityRef(a.getId())));
        }
