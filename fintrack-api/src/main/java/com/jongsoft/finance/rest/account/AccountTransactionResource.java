...
public class AccountTransactionResource {
  ...
  ResultPageResponse<TransactionResponse> search(
      @PathVariable long accountId, @Valid @Body AccountTransactionSearchRequest request) {
    ...
    var results = transactionProvider.lookup(command).map(TransactionResponse::new);
  ...
  TransactionResponse first(@PathVariable Long accountId, @Nullable String description) {
    var command = filterFactory.transaction().accounts(Collections.List(new EntityRef(accountId)));
  ...
  TransactionResponse update(
      @PathVariable long transactionId, @Valid @Body AccountTransactionCreateRequest request) {
