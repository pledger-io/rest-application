...
public class ScheduledTransactionResource {
    ...
    public List<ScheduledTransactionResponse> list() {
        return scheduleProvider.lookup().map(ScheduledTransactionResponse::new).toJava();
    }
    ...
    public ScheduledTransactionResponse create(
            @Valid @Body ScheduledTransactionCreateRequest request) {
        ...
        if (!account.isPresent() || !destination.isPresent()) {
            throw StatusException.badRequest(
                    "Either source or destination account cannot be located.");
        }
    ...
    public List<ScheduledTransactionResponse> search(@Valid @Body ScheduleSearchRequest request) {
        var filter =
                filterFactory
                        .schedule()
                        .contract(
                                Collections.List(request.getContracts())
                                        .map(c -> new EntityRef(c.id())))
                        .activeOnly();
    ...
    public ScheduledTransactionResponse patch(
            @PathVariable long scheduleId, @Valid @Body ScheduledTransactionPatchRequest request) {
        var scheduleOption = scheduleProvider.lookup().filter(s -> s.getId() == scheduleId);
        ...
        if (Objects.nonNull(request.getSchedule())) {
            schedule.adjustSchedule(
                    request.getSchedule().periodicity(), request.getSchedule().interval());
        }
    ...
    public void remove(@PathVariable long scheduleId) {
        var toRemove = scheduleProvider.lookup().filter(schedule -> schedule.getId() == scheduleId);
