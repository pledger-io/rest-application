...
public class AuthenticationHandler {
  ...
  public AuthenticationHandler(ApplicationEventPublisher<InternalAuthenticationEvent> eventPublisher) {
  ...
  void authenticated(LoginSuccessfulEvent event) {
    if (event.getSource() instanceof Authentication u) {
      eventPublisher.publishEvent(new InternalAuthenticationEvent(event.getSource(), u.getName()));
    }
  }
