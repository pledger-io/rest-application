...
public class TransactionProviderJpa implements TransactionProvider {
  ...
  protected Transaction convert(TransactionJournal source) {
    ...
    return Transaction.builder()
        ...
        .category(
            Control.Option(source.getCategory()).map(CategoryJpa::getLabel).getOrSupply(() -> null))
        .currency(source.getCurrency().getCode())
        .importSlug(
            Control.Option(source.getBatchImport()).map(ImportJpa::getSlug).getOrSupply(() -> null))
