...
public class TransactionFilterCommand extends JpaFilterBuilder<TransactionJournal>
    ...
    public TransactionProvider.FilterCommand range(Range<LocalDate> range) {
        query().condition(
                        Expressions.and(
                                Expressions.fieldCondition(
                                        "e", "date", FieldEquation.GTE, range.from()),
                                Expressions.fieldCondition(
                                        "e", "date", FieldEquation.LT, range.until())));
        return this;
    }
    ...
    public TransactionProvider.FilterCommand onlyIncome(boolean onlyIncome) {
        query().condition(
                        Expressions.fieldCondition(
                                "t",
                                "amount",
                                onlyIncome ? FieldEquation.GTE : FieldEquation.LTE,
                                0));
        return this;
    }
    ...
    public TransactionProvider.FilterCommand ownAccounts() {
        var types =
                Arrays.stream(SystemAccountTypes.values()).map(SystemAccountTypes::label).toArray();
        query().condition(
                        Expressions.fieldCondition(
