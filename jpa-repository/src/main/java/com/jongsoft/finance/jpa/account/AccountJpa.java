...
public class AccountJpa extends EntityJpa {
    ...
    @Formula(
            "(select max(tj.t_date) from transaction_part t join transaction_journal tj on tj.id ="
                    + " t.journal_id where t.account_id = id and t.deleted is null)")
    private LocalDate lastTransaction;

    @Basic(fetch = FetchType.LAZY)
    @Formula(
            "(select min(tj.t_date) from transaction_part t join transaction_journal tj on tj.id ="
                    + " t.journal_id where t.account_id = id and t.deleted is null)")
    private LocalDate firstTransaction;

    @Basic(fetch = FetchType.LAZY)
    @Formula(
            "(select sum(t.amount) from transaction_part t where t.account_id = id and t.deleted is"
                    + " null)")
