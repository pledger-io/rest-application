...
import com.jongsoft.finance.jpa.query.ComputationExpression;
...
public final class Expressions {
    ...
    public static BooleanExpression fieldLike(String tableAlias, String field, String value) {
        return new FieldCondition(tableAlias, generateRandomString(), field, FieldEquation.LIKE, "%" + value + "%");
    }
    ...
    public static <T> BooleanExpression fieldCondition(String tableAlias, String field, FieldEquation equation, T value) {
        return new FieldCondition(tableAlias,generateRandomString(), field, equation, value);
    }

    public static <T> BooleanExpression fieldBetween(String tableAlias, String field, T start, T end) {
    ...
    /**
     * Returns a computation expression representing the equality comparison between the left and right computation expressions.
     *
     * @param left the left computation expression to compare
     * @param right the right computation expression to compare
     * @return a computation expression representing the equality comparison between the left and right expressions
     */
    public static ComputationExpression equals(ComputationExpression left, ComputationExpression right) {
     ...
     */
    public static ComputationExpression caseWhen(BooleanExpression expression, ComputationExpression then, ComputationExpression elseValue) {
        return new ComputationExpression() {
            @Override
            public String hqlExpression() {
                return "CASE WHEN %s THEN %s ELSE %s END".formatted(expression.hqlExpression(), then.hqlExpression(), elseValue.hqlExpression());
            }
     ...
     * @return a new computation expression representing the addition of the two computation expressions
     */
    public static ComputationExpression addition(ComputationExpression left, ComputationExpression right) {
        return new ComputationExpression() {
            @Override
            public String hqlExpression() {
                return "(%s + %s)".formatted(left.hqlExpression(), right.hqlExpression());
