...
public class V20200429151821__MigrateEncryptedStorage extends BaseJavaMigration {
  ...
  public V20200429151821__MigrateEncryptedStorage() {
    //        this.securitySalt =
    // String.valueOf(Hex.encode(securitySalt.getBytes(StandardCharsets.UTF_8)));
  ...
  public void migrate(Context context) throws Exception {
    ...
    //        jdbcTemplate.query("select id, username, two_factor_secret from user_account",
    // this::processUser);
  }
  ...
  //
  //        jdbcTemplate.query("select id, file_code from import_config where user_id = " + id,
  // this::encrypt);
  ...
  //        try {
  //            final Path filePath = Paths.get(this.storageLocation + "/upload/" +
