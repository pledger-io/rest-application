...
public class V20200430171321__MigrateToEncryptedDatabase extends BaseJavaMigration {
    ...
    //        var name = API.Option(rs.getString("name")).map(encryptor::encrypt).getOrSupply(() ->
    // null);
    //        var iban = API.Option(rs.getString("iban")).map(encryptor::encrypt).getOrSupply(() ->
    // null);
    //        var bic = API.Option(rs.getString("bic")).map(encryptor::encrypt).getOrSupply(() ->
    // null);
    //        var number = API.Option(rs.getString("number")).map(encryptor::encrypt).getOrSupply(()
    // -> null);
    //
