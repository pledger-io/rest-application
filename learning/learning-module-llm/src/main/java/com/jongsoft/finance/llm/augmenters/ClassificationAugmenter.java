...
/**
 * A class that implements the RetrievalAugmentor interface to provide additional classification
 * augmentation capabilities for chat messages. It retrieves information from BudgetProvider,
 * CategoryProvider, and TagProvider to augment the user messages based on certain conditions.
 */
public class ClassificationAugmenter implements RetrievalAugmentor {
  ...
  private UserMessage augment(UserMessage userMessage, Metadata metadata) {
    ...
    if (currentMessage.contains("Pick the correct tags for a transaction on")) {
      ...
      allowedList = tagProvider.lookup().map(Tag::name).stream().collect(Collectors.joining(","));
    }
