...
public class OccurrencePattern implements Pattern {
  ...
  private double calculateConfidence(List<EmbeddingMatch<TextSegment>> matches) {
    ...
    double matchFactor = Math.min(1.0, matches.size() / 10.0); // Scale based on number of matches, max at 10
  ...
  private PatternType detectMonthlyOrWeekly(List<Number> intervals) {
    ...
    var avgInterval = (int) intervals
        .stream()
        .mapToLong(Number::longValue)
        .average()
        .orElse(0);
    var numberWithinAvg = intervals
        .stream()
        .mapToLong(Number::longValue)
    ...
    log.trace("Average interval: {}, number within avg: {}, consistent: {}.", avgInterval, numberWithinAvg, isConsistent);
    if (!isConsistent) {
      return null;
    }
