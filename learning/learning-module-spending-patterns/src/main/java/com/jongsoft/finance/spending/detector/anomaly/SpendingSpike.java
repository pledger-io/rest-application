...
public class SpendingSpike implements Anomaly {
  ...
  public SpendingSpike(FilterFactory filterFactory, TransactionProvider transactionProvider, BudgetProvider budgetProvider) {
  ...
  public Optional<SpendingInsight> detect(Transaction transaction, UserCategoryStatistics statistics) {
    ...
    var avgMonthlySpending = monthlyMap.values()
        .stream()
        .mapToDouble(Double::doubleValue)
        .average().orElse(0.0);

    String currentMonth = transaction.getDate().getYear() + "-" + transaction.getDate().getMonthValue();
  ...
  public Map<String, Double> computeSpendingPerMonth(Transaction transaction, int lastNumberOfMonths) {
    ...
    for (int i = 0; i < lastNumberOfMonths; i++) {
      ...
      computedBalance.ifPresent(amount ->
          monthlyMap.put(
